import { PipeTypes, Given, PipeObject, Test, Transform } from "./types";

const given = (initialTest?: Test): Given => {
  const pipe = <Array<PipeObject>>[];
  if (initialTest) {
  ...
  const myGiven = <Given>{
    ...
    apply: (obj: object) => {
      ...
      const result = pipe.reduce(
        (inputValue, pipeObject) => {
          if (
            pipeObject.pipeType === PipeTypes.when &&
